...
const Tooltip: React.FC<TooltipProps> = ({ text, alignment = "left", type = "sosmed", children }) => {
  return (
    <div className="relative inline-block group z-50">
      {children}
      <div
        className={`absolute top-1/2 transform -translate-y-1/2  whitespace-nowrap opacity-0 pointer-events-none transition-opacity duration-200 group-hover:opacity-100 group-hover:pointer-events-auto ${alignment === "bottom"
            ? "top-full mt-6"
            : alignment !== "right"
              ? "left-auto right-full mr-5"
              : "left-full ml-5"
